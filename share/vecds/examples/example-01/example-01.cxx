...
{ ifstream infile;
  ...
  while ( !infile.eof() ) {
     ...
     if ( instream >> a_name1 >> xx >> yy >> zz ) {
        ...
        if ( !instream.eof() )  cout << "Warning. Too much items on the line nr " << count+1 << endl;} 
     else {
        if ( (count-2)>=atoms->n_atoms ) return true;
...
{ cout << "multdisl for atom nr. " << n_a << ", num_rep=" << num_rep << ", distance=" << distance << endl;
  for (int i=0; i<n_Atoms; i++) atoms->du[i] = glm::dvec3(0., 0., 0.);
...
//  extPrint(nazwa.str().c_str(), atoms->n_atoms, atoms->du);
  return;
}
...
{ int numbFields;
  ...
  while ( !infile.eof() ) {
    ...
    if ( fields[0].compare(string("//"))==0 ) continue;
...
//        normalizeCoordinates(atoms->u);
    ...
    ostringstream nazwa;
    switch (what) {
    case 0 : if ( numbFields!=6 ) {       // latt_const
	cout << "error 0" << endl;
	return false;}
      ...
      if ( crysCell->gamma==90. ) { cosa6 = 0.;  sina6 = 1.; }
      cosa5 = cos(deg2rad*crysCell->beta);
      cosa4 = cos(deg2rad*crysCell->alpha);
      if ( crysCell->alpha==90. ) cosa4 = 0.;
      if ( crysCell->beta==90. ) cosa5 = 0.;
...
//      printMat("c2o  - ", crysCell->c2o);
      crysCell->o2c = glm::inverse(crysCell->c2o);
//      printMat("o2c  - ", crysCell->o2c);
      what = -1;
      break;
    case 1 : if ( numbFields!=1 ) {         // numb_addAtoms
	cout << "error 1" << endl;
	return false;}
      ...
      what = -1;
      break;
    case 2 : if ( numbFields!=2 ) {         // single_disl
	cout << "error 2" << endl;
	return false;}
      numMark = toInt(fields[0]);
      if ( numMark<0 ) { sw_iter = false; numMark = -numMark; } else sw_iter = true;
      act_disl = fields[1];
      processMiller(act_disl);
      for (int i=0; i<n_Atoms; i++) {atoms->du[i] = glm::dvec3(0.,0.,0.); atoms->beta[i] = glm::dmat3(0.,0.,0., 0.,0.,0., 0.,0.,0.);}
      singledisl(numMark);
      ++n_dislocations;

      sumU = glm::dvec3(0.0, 0.0, 0.0);;
      for (int i=0; i<n_Atoms; i++) sumU += atoms->du[i];
      sumU /= double(n_Atoms);
      cout << "Dislocation nr. " << n_dislocations << "   sumU = ( " << sumU.x << ", " << sumU.y << ", " << sumU.z << " )" << endl;
      for (int i=0; i<n_Atoms; i++) atoms->du[i] -= sumU;
      ...
      what = -1;
      break;
    case 3 : if ( numbFields!=4 ) {          // mult_disl
	cout << "error 3" << endl;
	return false; }
      numMark = toInt(fields[0]);
      act_disl = fields[1];
...
//      normalizeCoordinates(atoms->u);
      for (int i=0; i<n_Atoms; i++)  atoms->u[i] += atoms->du[i];
      for (int i=0; i<n_Atoms; i++) atoms->du[i] = glm::dvec3(0., 0., 0.);
      ++n_dislocations;
      what = -1;
      break;
    default: cout << "Warning - unrecognized line nr. " << count << endl;
      break; }}
