...
public:
    T x,y,z;
    istream& read( istream &in );
    void MaxMins( double &minX, double &maxX, double &minY, double &maxY, double &minZ, double &maxZ );

};

template <class T>
void point3D<T>::MaxMins( double &minX, double &maxX, double &minY, double &maxY, double &minZ, double &maxZ  )
{
...
template < class T >
void GraphData<T>::MaxMins( double &minX, double &maxX, double &minY, double &maxY, double &minZ, double &maxZ )
{
    unsigned k,l;
    for (k=0; k<trs3D.size(); k++)
    {
        for (l=0; l<trs3D[k].size; l++ )
        {
            trs3D[ k ].data[l].p1.MaxMins( minX, maxX, minY, maxY, minZ, maxZ );
            trs3D[ k ].data[l].p2.MaxMins( minX, maxX, minY, maxY, minZ, maxZ );
            trs3D[ k ].data[l].p3.MaxMins( minX, maxX, minY, maxY, minZ, maxZ );
        }
    }

    for (k=0; k<lns3D.size(); k++)
    {
        for (l=0; l<lns3D[k].size; l++ )
        {
            lns3D[ k ].data[ l ].p1.MaxMins( minX, maxX, minY, maxY, minZ, maxZ );
            lns3D[ k ].data[ l ].p2.MaxMins( minX, maxX, minY, maxY, minZ, maxZ );
        }
    }

    for (k=0; k<pts3D.size(); k++)
    {
        for (l=0; l<pts3D[k].size; l++ )
           pts3D[ k ].data[ l ].MaxMins( minX, maxX, minY, maxY, minZ, maxZ );
    }
...
istream& GraphData<T>::read( istream &in )
{
    ...
    in >> size;
    strS.resize(size);
    strX.resize(size,vector<double>(3));
    for ( k=0; k<size; k++)
        in >> strX[k][0] >> strX[k][1] >> strX[k][2] >> strS[k];
    return in;
}
