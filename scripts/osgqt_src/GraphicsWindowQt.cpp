...
GLWidget::GLWidget( QWidget* parent, Qt::WindowFlags f, bool forwardKeyEvents )
...
void GLWidget::mousePressEvent( QMouseEvent* event )
{
    ...
    setKeyboardModifiers( event );
    _gw->getEventQueue()->mouseButtonPress( event->position().x()*_devicePixelRatio, event->position().y()*_devicePixelRatio, button );
}
...
void GLWidget::mouseReleaseEvent( QMouseEvent* event )
{
    ...
    setKeyboardModifiers( event );
    _gw->getEventQueue()->mouseButtonRelease( event->position().x()*_devicePixelRatio, event->position().y()*_devicePixelRatio, button );
}
...
void GLWidget::mouseDoubleClickEvent( QMouseEvent* event )
{
    ...
    setKeyboardModifiers( event );
    _gw->getEventQueue()->mouseDoubleButtonPress( event->position().x()*_devicePixelRatio, event->position().y()*_devicePixelRatio, button );
}

void GLWidget::mouseMoveEvent( QMouseEvent* event )
{
    setKeyboardModifiers( event );
    _gw->getEventQueue()->mouseMotion( event->position().x()*_devicePixelRatio, event->position().y()*_devicePixelRatio );
}
    ...
    if (!delta.isNull()) {
        _gw->getEventQueue()->mouseScroll(
            delta.y() != 0
                ...
                : (delta.x() > 0 ? osgGA::GUIEventAdapter::SCROLL_LEFT : osgGA::GUIEventAdapter::SCROLL_RIGHT));
    }
}
...
GraphicsWindowQt::GraphicsWindowQt( osg::GraphicsContext::Traits* traits, QWidget* parent, QWidget* shareWidget, Qt::WindowFlags f )
:   _realized(false)
{
...
bool GraphicsWindowQt::init( QWidget* parent, QWidget* shareWidget, Qt::WindowFlags f )
{
    ...
    if ( !_widget )
    {
        ...
        // create widget
        _widget = new GLWidget( parent, flags );
        _widget->setFormat( traits2qsurfaceFormat( _traits.get() ) );
    }
...
QSurfaceFormat GraphicsWindowQt::traits2qsurfaceFormat( const osg::GraphicsContext::Traits* traits )
{
    ...
    // Use sensible minimums so GLX can always find a matching FBConfig
    format.setRedBufferSize( traits->red > 0 ? traits->red : 8 );
    format.setGreenBufferSize( traits->green > 0 ? traits->green : 8 );
    format.setBlueBufferSize( traits->blue > 0 ? traits->blue : 8 );
    if ( traits->alpha > 0 ) format.setAlphaBufferSize( traits->alpha );
    format.setDepthBufferSize( traits->depth > 0 ? traits->depth : 24 );
    if ( traits->stencil > 0 ) format.setStencilBufferSize( traits->stencil );
    if ( traits->samples > 0 ) format.setSamples( traits->samples );

    // QOpenGLWidget always needs DoubleBuffer for correct FBO compositing
...
{
    // QOpenGLWidget renders to an FBO and composites automatically; update() triggers repaint.
    _widget->update();
